(function($) {
    $.fn.GoogleMapEditor = function(options) {
        var defaults = {
            ...
            markerPinFiles: [
                "flag-azure.png", /* List of files to be available from custom marker icons path */
                "flag-green.png",
                "needle-pink.png",
                "niddle-green.png",
                "pin-azure.png",
            ...
            language:
                "en", /* List of supported languages https://spreadsheets.google.com/spreadsheet/pub?key=0Ah0xU81penP1cDlwZHdzYWkyaERNc0xrWHNvTTA1S1E&gid=1 */
            singleLocation: false, /* Allow only one location present on the map */
            searchBox: true, /* Add search box to the map */
            drawingTools: [
                "marker", /* Tools to be availale for editing */
                "polyline",
                "polygon",
                "circle",
                "rectangle"
            ...
            scrollWheel: true, /* Use mouse wheel to zoom in and zoom out*/
            style:
                null, /* Build custom syles at http://gmaps-samples-v3.googlecode.com/svn/trunk/styledmaps/wizard/index.html */
            stylesPath:
                ".../styles.json", /* Url of JSON file with map styles definitions. Enpowered by snazzymaps.com */
            locations: [], /* Locations to be loaded with the map */
            routeId: 0,
            dataChange: null, /* Event raised when anything on the map changed */
            locationCheck: function(map, location) { /* Event raised when a location on the map is checked */
                $(document).trigger("location-checked",
                    [map, location]);
            },
            locationUncheck: function(map, location) { /* Event raised when a location on the map is unchecked*/
                $(document).trigger("location-unchecked",
                    [map, location]);
            },
            locationNew: function(map, location) { /* Event raised when new location is added on the map */
                $(document).trigger("location-added",
                    [map, location]);
            },
            locationUpdate: function(map, location) { /* Event raised when location is updated on the map */
                $(document).trigger("location-updated",
                    [map, location]);
            },
            locationDelete: function(map, location) { /* Event raised when location is deleted from the map */
                $(document).trigger("location-deleted",
                    [map, location]);
            },
            locationMove: null, /* Event reaised when location is moved on the map to a different position */
            idcounter: 0 /*Global counter for setting id for new locations*/
        };

        var settings = $.extend({}, defaults, options);
        var mapApiUrl =
            "//maps.googleapis.com/maps/api/js?key=AIzaSyBsHtuV0BERSPzwFSHpkl5k428uZ36WS7g&sensor=false&callback=mapApiLoaded&libraries=drawing,places";
        ...
        function addSearchBox(map) {
            ...
            $(map.container).parent().prepend("<input id=\"" +
                inputId +
                "\" style=\"margin-top:4px\" type=\"text\" placeholder=\"Search Box\" />");
        ...
        function addFitBoundsButton(map) {
            ...
            $(map.container).parent()
                .prepend("<input id=\"" + inputId + "\" class=\"fit-bound\" type=\"button\" title=\"Fit bounds\" />");
            var input = document.getElementById(inputId);
            if (input != null) {
        ...
        function addCurrentLocationButton(map) {
            ...
            $(map.container).parent().prepend("<input id=\"" +
                inputId +
                "\" class=\"current-location\" type=\"button\" title=\"My Location\" />");
            var input = document.getElementById(inputId);
            if (input != null) {
                map.controls[google.maps.ControlPosition.TOP_CENTER].push(input);
                $(input).click(function() {
                    if (navigator.geolocation) {
                        ...
                        navigator.geolocation.getCurrentPosition(function(position) {
                                ...
                                if (settings.editMode) {
                                    var myLocation = new Location(settings.idcounter++,
                                        settings.routeId,
                                    ...
                                    locationBounds = new google.maps.LatLngBounds(
                                        new google.maps.LatLng(myLocation.Coordinates[0].Latitude,
                                            myLocation.Coordinates[0].Longitude));
                                    ...
                                    if (settings.locationNew != null && typeof (settings.locationNew) == "function") {
                                        ...
                                        settings.locationNew(map, myLocation);
                                    }
                                    saveToJson(map);
                                } else {
                                    locationBounds = new google.maps.LatLngBounds(
        ...
        function initializeGoogleMapEditor(container) {
            ...
            $("<div/>", { class: "map-overlay" })
                .append($("<input/>", { type: "button", value: "Close", class: "btn-close" }))
                .insertAfter($(map.container));
            ...
            if (settings.editMode) {
                ...
                google.maps.event.addListener(drawingManager,
                    ...
                    function(e) {
                        ...
                        if (settings.locationNew != null && typeof (settings.locationNew) == "function") {
                            new google.maps.event.trigger(location.Overlay, 'click');
                            settings.locationNew(map, location);
                        }
                    });
                drawingManager.setMap(map);

                // #region RoutePageButtonEventsHandlers
                $(document).on("save-route-btn-clicked",
                    function(parameters) {
                        var newLocations = [];
                        $.each(map.locations,
                            function(prop, value) {
                                var loc = $.extend(true, {}, value);
                                loc.Overlay = null;
                                $.each(loc.Coordinates,
                                    function(prop, value) {
                        ...
                        if (!ValidateLocations(newLocations)) {
                            alert("Please, fill in Name and Description for all locations.");
                            return;
                        }

                        $.ajax({
                            url: "/Location/SaveLocations",
                            method: "POST",
        ...
        function attachTransformHandlers(map, overlay, type) {
            ...
            case google.maps.drawing.OverlayType.POLYGON:
                google.maps.event.addListener(overlay.getPath(),
                    'set_at',
                    function(index, obj) {
        ...
        function initLocationObject(map, overlay, type) {
            ...
            case google.maps.drawing.OverlayType.MARKER:
                maplocation = new Location(settings.idcounter++,
                    settings.routeId,
                    [new Coordinate(overlay.getPosition().lat(), overlay.getPosition().lng())],
                    google.maps.drawing.OverlayType.MARKER,
                    "",
                    "",
                    "",
                ...
                break;
            case google.maps.drawing.OverlayType.CIRCLE:
                maplocation = new Location([new Coordinate(overlay.getCenter().lat(), overlay.getCenter().lng())],
            ...
            case google.maps.drawing.OverlayType.POLYGON:
                ...
                break;
            case google.maps.drawing.OverlayType.RECTANGLE:
                coordinates.push(new Coordinate(overlay.getBounds().getNorthEast().lat(),
                    overlay.getBounds().getNorthEast().lng()));
                coordinates.push(new Coordinate(overlay.getBounds().getSouthWest().lat(),
                    overlay.getBounds().getSouthWest().lng()));
        ...
        function initMapObject(map, location) {
            ...
            case google.maps.drawing.OverlayType.MARKER:
                ...
                var marker = new google.maps.Marker({
                    draggable: settings.editMode,
                    map: map,
                    position: new google.maps.LatLng(location.Coordinates[0].Latitude,
            ...
            case google.maps.drawing.OverlayType.POLYLINE:
                ...
                for (c = 0; c < location.Coordinates.length; c++) {
                    var polylineCorner =
                        new google.maps.LatLng(location.Coordinates[c].Latitude, location.Coordinates[c].Longitude);
        ...
        function saveToJson(map) {
            ...
            if (settings.editMode) {
                ...
                var properties = [
                    "editMode", "editTemplatesPath", "markerPinsPath", "markerPinFiles", "drawingBorderColor",
                    "drawingBorderWidth", "drawingFillColor", "zoom", "width", "height", "singleLocation", "center",
                    "language", "searchBox", "drawingTools", "zoomControl", "panControl", "scaleControl",
                    "streetViewControl", "scrollWheel", "stylesPath", "style", "locations", "routeId"
                ];
                properties = properties.concat([
                    "latitude", "longitude", "Coordinates", "Latitude", "Longitude", "Radius", "LocationType", "Icon",
                    "HoverIcon", "BorderColor", "BorderWeight", "FillColor", "Id", "Name", "Description"
                ]);
                properties = properties.concat([
                    "featureType", "elementType", "stylers", "color", "hue", "weight", "visibility", "lightness",
                    "saturation"
                ]);
        ...
        function handleMapCloseClick(map, type) {
            ...
            if (settings.locationUncheck != null && typeof (settings.locationUncheck) == "function") {
                settings.locationUncheck(map, map.activeLocation);
            }
        }

        function attachLocationHandlers(map, location, type) {
            google.maps.event.addListener(location.Overlay,
                'dragend',
                function(event) {
            ...
            google.maps.event.addListener(location.Overlay,
                ...
                function() {
                    if (type == google.maps.drawing.OverlayType.MARKER) {
                        if (location.HoverIcon &&
                            location.HoverIcon != 'none' &&
            ...
            google.maps.event.addListener(location.Overlay,
                ...
                function() {
                    ...
                    google.maps.event.addListener(map.infoWindow,
                        'closeclick',
                        function() {
                    ...
                    if (contentObj != null && settings.editMode) {
                        ...
                        map.infoWindow.setContent($("<div/>")
                            .append($("<div/>", { class: "popup-message" }).html(location.Description)).html());
                    }

                    google.maps.event.addListener(map.infoWindow,
                        'domready',
                        function() {
                            $(map.container).find('.popup-content input[name="locationName"]').val(map.activeLocation.Name);
                            $(map.container).find('.popup-content textarea[name="locationDescription"]').val(map.activeLocation.Description);
                            ...
                            $(map.container).find('.popup-content input[name="borderWidth"]')
                                .val(location.BorderWeight);
                            ...
                            var markerHoverIconsList =
                                $(map.container).find('.popup-content select[name="hover-icon"]');
                            ...
                            var borderColorInputPicker =
                                $(map.container).find('.popup-content input[name="strokeColorPicker"]');
                            var fillColorInputPicker =
                                $(map.container).find('.popup-content input[name="fillColorPicker"]');
                    ...
                    if (settings.locationCheck != null && typeof (settings.locationCheck) == "function") {
                        settings.locationCheck(map, location);
                    }
        ...
        function updateLocationObject(map, location, type, showBallonInfo) {
            ...
            if ($('.popup-content').length > 0) {
                ...
                location.Name = $(map.container).find('.popup-content input[name="locationName"]').val();
                location.Description = $(map.container).find('.popup-content textarea[name="locationDescription"]').val();
            ...
            case google.maps.drawing.OverlayType.MARKER:
                ...
                location.Coordinates.push(new Coordinate(location.Overlay.getPosition().lat(),
                    location.Overlay.getPosition().lng()));
                if ($('select[name="icon"]').length > 0) {
                    ...
                    location.Name = $('.popup-content input[name="locationName"]').val();
                    location.Description = $('.popup-content textarea[name="locationDescription"]').val();
                    ...
                    location.Overlay.setPosition(new google.maps.LatLng(location.Coordinates[0].Latitude,
                        location.Coordinates[0].Longitude));
            ...
            case google.maps.drawing.OverlayType.CIRCLE:
                ...
                if ($('.popup-content input[name="centerLat"]').length > 0 &&
                    $('.popup-content input[name="centerLng"]').length > 0) {

                    google.maps.event.clearListeners(location.Overlay, 'center_changed');
                    location.Overlay.setCenter(new google.maps.LatLng(
                        parseFloat($('.popup-content input[name="centerLat"]').val()),
                        parseFloat($('.popup-content input[name="centerLng"]').val())));
                    google.maps.event.addListener(location.Overlay,
                        'center_changed',
                        function(index, obj) {
                            updateLocationObject(map, location, type, true);
                    ...
                    location.Coordinates.length = 0;
                    location.Coordinates.push(new Coordinate(location.Overlay.getCenter().lat(),
                        location.Overlay.getCenter().lng()));
            ...
            case google.maps.drawing.OverlayType.POLYLINE:
                for (i = 0; i < location.Overlay.getPath().length; i++) {
                    coordinates.push(new Coordinate(location.Overlay.getPath().getAt(i).lat(),
                        location.Overlay.getPath().getAt(i).lng()));
                }
            ...
            case google.maps.drawing.OverlayType.POLYGON:
                ...
                break;
            case google.maps.drawing.OverlayType.RECTANGLE:
                coordinates.push(new Coordinate(location.Overlay.getBounds().getNorthEast().lat(),
                    location.Overlay.getBounds().getNorthEast().lng()));
                coordinates.push(new Coordinate(location.Overlay.getBounds().getSouthWest().lat(),
                    location.Overlay.getBounds().getSouthWest().lng()));
                location.Coordinates = coordinates;
                break;
            }
            ...
            if (settings.locationUpdate != null && typeof (settings.locationUpdate) == "function") {
                settings.locationUpdate(map, location);
            }
        ...
        function initPopupButtons(map) {
            ...
            deleteButton.click(function() {
                if (confirm("Delete this location?")) {
                    var locationId = map.activeLocation.Id;
                    ...
                    if (settings.locationDelete != null && typeof (settings.locationDelete) == "function") {
                        settings.locationDelete(map, locationId);
                    }
                }
            });
            ...
            $(
                    '.popup-content input[type="text"],.popup-content input[type="number"],.popup-content input[type="color"],.popup-content select,.popup-content textarea')
                .change(function() {
            ...
            if (map.locations != null && map.locations.length > 0) {
                ...
                for (i = 0; i < map.locations.length; i++) {
                    for (j = 0; j < map.locations[i].Coordinates.length; j++) {
                        mapLocationBounds.extend(new google.maps.LatLng(map.locations[i].Coordinates[j].Latitude,
                            map.locations[i].Coordinates[j].Longitude));
                    }
                }
        ...
        function Location(id, routeId, coordinates, locationtype, name, description) {
            if (id == 0) {
                if (settings.locations.length > 0) {
                    settings.idcounter = getMaxId();
                } else {
                    id = 1;
                }
                settings.idcounter++;
        ...
        function getMaxId() {
            for (var i = 0; i < settings.locations.length; i++) {
                if (settings != null &&
                    ...
                    settings.locations[i] != null &&
                    settings.locations[i].id != null &&
                    settings.locations[i].id != 'undefined' &&
                    idcounter < settings.locations[i].id) {
                    idcounter = settings.locations[i].id;
                }
            }
        }
            ...
            $.each(locationList,
                function(prop, loc) {
                    if (loc.Name == '' || loc.Description == '') {
                        success = false;
                        message += "Location " + loc.Id + " is invalid.";
                    }
                });
...
/* Simple color picker https://github.com/rachel-carvalho/simple-color-picker */
    $.fn.simpleColorPicker = function(options) {
        var defaults = {
            colorsPerLine: 8,
            colors: [
                '#000000', '#444444', '#666666', '#999999', '#cccccc', '#eeeeee', '#f3f3f3', '#ffffff', '#ff0000',
                '#ff9900', '#ffff00', '#00ff00', '#00ffff', '#0000ff', '#9900ff', '#ff00ff', '#f4cccc', '#fce5cd',
                '#fff2cc', '#d9ead3', '#d0e0e3', '#cfe2f3', '#d9d2e9', '#ead1dc', '#ea9999', '#f9cb9c', '#ffe599',
                '#b6d7a8', '#a2c4c9', '#9fc5e8', '#b4a7d6', '#d5a6bd', '#e06666', '#f6b26b', '#ffd966', '#93c47d',
                '#76a5af', '#6fa8dc', '#8e7cc3', '#c27ba0', '#cc0000', '#e69138', '#f1c232', '#6aa84f', '#45818e',
                '#3d85c6', '#674ea7', '#a64d79', '#990000', '#b45f06', '#bf9000', '#38761d', '#134f5c', '#0b5394',
                '#351c75', '#741b47', '#660000', '#783f04', '#7f6000', '#274e13', '#0c343d', '#073763', '#20124d',
                '#4C1130'
            ],
            showEffect: '',
            hideEffect: '',
